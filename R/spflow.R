...
spflow <- function(
  ...
  assert(id_spflow_pairs %in% pair_ids,
         'The the spflow_pairs with id "%s" are not available!',
         id_spflow_pairs)


  od_id <- id(pull_member(spflow_multinet, id_spflow_pairs))
  estimation_control <- enhance_spflow_control(
    estimation_control = estimation_control,
    is_within = od_id["orig"] == od_id["dest"])

  spflow_data <- pull_spflow_data(spflow_multinet, id_spflow_pairs)
  spflow_neighborhood <- pull_spflow_neighborhood(spflow_multinet, id_spflow_pairs)
