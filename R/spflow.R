...
#'   A [spflow_multinet()] object that contains information on the origins,
#'   and the destinations and their neighborhood structure
#' @param network_pair_id
#'   A character indicating the id of a [spflow_pairs()] (only relevant if
#'   the [spflow_multinet()] contains multiple `spflow_pairs`-objects:
...
#' [spflow_multinet()] object, which gathers information on the origins and
#' destinations (inside [spflow_nodes()] objects), as well as the
#' information on the origin-destination pairs (inside a [spflow_pairs()]
#' object).
#'
...
spflow <- function(
    flow_formula,
    spflow_multinet,
    network_pair_id = id(spflow_multinet)[["network_pairs"]][[1]],
  ...
  pair_ids <- id(spflow_multinet)[["network_pairs"]]
  ...
  od_id <- id(pull_member(spflow_multinet, network_pair_id))
  estimation_control <- enhance_spflow_control(
    flow_control = flow_control,
    is_within = od_id["orig"] == od_id["dest"])

  spflow_data <- pull_spflow_data(spflow_multinet, network_pair_id)
  spflow_neighborhood <- pull_spflow_neighborhood(spflow_multinet, network_pair_id)
