...
#'   A character indicating the id of a [spflow_network_pairs()] (only relevant if
#'   the [spflow_network_multi()] contains multiple `spflow_network_pairs`-objects:
...
#' destinations (inside [spflow_network_nodes()] objects), as well as the
#' information on the origin-destination pairs (inside a [spflow_network_pairs()]
#' object).
#'
...
validate_networks <- function(
  ...
  assert(id_spflow_pairs %in% id(spflow_networks)[["pairs"]],
         'The the spflow_network_pairs with id "%s" are not available!',
         id_spflow_pairs)
