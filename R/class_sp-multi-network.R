...
setMethod(
  ...
  function(object,
    ...
    orig_data <- dat(object,network_id = od_ids["origin_network_id"])
    names(orig_data) <- "orig_" %p% names(orig_data)

    dest_data <- dat(object,network_id = od_ids["destination_network_id"])
...
sp_multi_network <- function(...) {
  ...
  names(sp_network_pairs) <- lapply(sp_network_pairs, id, "network_pair_id")
