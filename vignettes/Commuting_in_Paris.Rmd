...
```{r}
drop_area <- names(paris10km_nodes != "AREA")
plot(paris10km_nodes[drop_area])
```
...
mid_points <- suppressWarnings({
    st_point_on_surface(st_geometry(paris10km_nodes))})
...
plot(st_geometry(paris10km_nodes))
...
paris10km_net <- 
  sp_network_nodes(
    ...
    node_neighborhood = nb2mat(paris10km_nb$by_border),
    node_data = st_drop_geometry(paris10km_nodes),
    node_id_column = "ID")
