...
Before estimation we should also investigate the correlation structure of the data.
The `pair_corr()` method creates a correlation matrix and the `corr_map()` function allows to plot it.
To clarify which variables should be looked at we use a formula, by default all variables are used.


```{r, out.width = "8.5in"}
cor_formula <- log(1 + COMMUTE_FLOW) ~ . + P_(log( 1 + DISTANCE))
cor_mat <- pair_corr(paris10km_multinet, spflow_formula = cor_formula, add_lags_x = FALSE)
colnames(cor_mat) <- substr(colnames(cor_mat),1,3)
corr_map(cor_mat)
```
...
Estimation with default settings requires two arguments: an `spflow_network_multi-class` and a `spflow_formula`.
...
### Some simple diagonstics

Calling `plot(results_mcmc)` would create a whole sequence of graphics that allow to diagnose the fit.
We will first look at the pairwise correlations of the model data.
Here we see, for example, that the residuals and their spatial lags are not correlated with the explanatory variables.

```{r, out.width = "8.5in"}
res_corr <- pair_corr(results_mcmc)
colnames(res_corr) <- substr(colnames(res_corr),1,3)
corr_map(res_corr)
```

We can furthermore create Moran scatter plots to check whether the residuals still exhibit spatial autocorrelation with respect to the three potential neighborhood matrices $W_d$, $W_o$ and $W_w$.

```{r}
spflow_moran_plots(results_mcmc)
```

A quick investigation of the 1% residuals with largest magnitude reveals that long distances seem to be predicted with lower precision.
...
spflow_map(
  ...
  add = TRUE,
  legend_position = "bottomleft",
  filter_lowest = .99, # concentrate on the 1% largest
  remove_intra = TRUE) # intra-regional flows are too large
```
...
### Evaluating the impact of variables

Finally we can evaluate the impact certain characteristics have on the outcome.
Here we look at a scenario where the population in the center of Paris is doubled, which strongly increases the outflow from this area and decreases the inflow to it. 

```{r}
change_mun <- "75101"
change_paris <- dat(paris10km_multinet, "paris")
change_paris <- change_paris[change_paris$ID_MUN == change_mun,]
change_paris[,"POPULATION"] <- change_paris[,"POPULATION"]*2
change_paris <- change_paris[,1:2]

effect_matrix <- predict_effect(results_mcmc,new_data = list("paris" = change_paris), return_type = "M")
inout_effects <- cbind(
  paris10km_municipalities,
  OUTFLOW = colSums(effect_matrix),
  INFLOW = rowSums(effect_matrix))
plot(inout_effects[,c("OUTFLOW", "INFLOW")])
