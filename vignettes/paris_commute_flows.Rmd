...
To describe the nodes of a network the package provides `spflow_nodes-class` that combines attributes of the nodes with the chosen network structure.
For our model we choose the contiguity based neighborhood structure.

```{r}
paris10km_net <- spflow_nodes(
    id_spflow_nodes = "paris",
...
The `spflow_pairs-class` contains all information on the pairs of nodes belonging to the origin and destination networks.
...
The `spflow_multinet-class` combines information on the nodes and the node-pairs and also ensures that both data sources are consistent.
For example, if some of the origins in the `spflow_pairs-class` are not identified with the nodes in the `spflow_nodes-class` an error will be raised.
...
spflow_map(
  ...
  filter_lowest = .9, # concentrate on the 10% largest
...
Estimation with default settings requires two arguments: an `spflow_multinet-class` and a `spflow_formula`.
...
results_default <- spflow(
  ...
  spflow_multinet = paris10km_multinet)
...
```{r}
qqnorm(resid(results_mcmc))
qqline(y = resid(results_mcmc), col = "red")
```

We can Moran scatter plots to check whether the residual still exhibits spatial autocorrelation, linked to the three different types of neighborhoods.

```{r}
spflow_moran_plots(results_mcmc)
```

We can quickly investigate the largest 1% of residuals, which seem to occur for long distances. 
...
This can be achieved using by using the `weight_variable` option in `spflow_control()`, but is left out in this introductory vignette.
